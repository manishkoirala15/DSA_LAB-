...
int m1 = 0, m2 = 1; 
if(nodes[m2]->f < nodes[m1]->f) { int t=m1; m1=m2; m2=t; } 
for(int i = 2; i < n; i++) { 
if(nodes[i]->f < nodes[m1]->f) { m2 = m1; m1 = i; } 
else if(nodes[i]->f < nodes[m2]->f) m2 = i; 
} 
...
Node* nodes[256]; int n = 0; 
for(int i = 0; i < 256; i++) 
if(freq[i]) nodes[n++] = newNode((char)i, freq[i]); 
int arr[100]; 
